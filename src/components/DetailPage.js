...
export function DetailPage() {
  ...
  React.useEffect(() => {
    async function fetchData() {
      ...
      let [papers, stages, analyticStages, statisticians, paperTypes, dataFocus, paperFocus, 
          sigs, alphaAuthors, consortia, sponsoringPI, studies] =
        await Promise.all([papersPromise, stagesPromise, analyticStagesPromise, statisticiansPromise,
          paperTypesPromise, dataFocusPromise, paperFocusPromise, sigsPromise, alphaAuthorsPromise,
      ...
      <TabView>
        <TabPanel header="Paper Detail">
          <div className="container">
            {data && (
              <EditPaper editItem={data} stages={stages} analyticStages={analyticStages} statisticians={statisticians}
                paperTypes={paperTypes} dataFocus={dataFocus} paperFocus={paperFocus} sigs={sigs} 
                alphaAuthors={alphaAuthors} consortia={consortia} sponsoringPI={sponsoringPI} studies={studies}
                options={options}
                doSubmit={doSubmit} cancelEdit={() => setData(null)} />
            )}
          </div>
        </TabPanel>
          ...
          <div className="form-row">
            <div className="form-group col-md-4">
              ...
              <Field name="CONVENER" placeholder="Convener" className="form-control" id="convenerInput"
                     value={values.CONVENER || ''} >
              </Field>
            </div>
            <div className="form-group col-md-4">
              <label htmlFor="sponsorPIInput" >Sponsoring PI </label>
              <Field as="select" name="SPONSORING_PI" placeholder="Sponsoring PI" className="form-control"  id="sponsorPIInput"
                     value={values.SPONSORING_PI || ''} >
              {props.sponsoringPI && props.sponsoringPI.map(row => {
                  return <option key={row.SPONSOR_ID}
                    value={row.SPONSOR_ID === null ? '' : row.SPONSOR_ID}
                    label={row.SPONSOR} />
                })
                }                
              </Field>
            ...
            <div className="form-group col-md-4">
              ...
              <Field as="select" name="CONSORTIUM_ID" placeholder="Consortium" className="form-control" id="consortiumIDInput"
               value={values.CONSORTIUM_ID || ''}>
              {props.consortia && props.consortia.map(row => {
                  return <option key={row.CONSORTIUM_ID}
                    value={row.CONSORTIUM_ID === null ? '' : row.CONSORTIUM_ID}
                    label={row.CONSORTIUM_NAME + " (" + row.CONSORTIUM_PI + ")"} />
                })
                }
                </Field>
            </div>
          ...
          <div className="form-row">
            <div className="form-group col-md-4">
              ...
              <Field as="select" name="STAGE_ID" placeholder="Stage ID" className="form-control" id="stageInput">
                {props.stages && props.stages.map(row => {
                  return <option key={row.STAGE_ID}
                    value={row.STAGE_ID === null ? '' : row.STAGE_ID}
                    label={row.NAME} />
                })
                }
              </Field>
            ...
            <div className="form-group col-md-4">
              ...
              <Field as="select" name="ANALYTIC_STAGE" placeholder="Analytic Stage" className="form-control" id="analyticStageInput"
                     value={values.ANALYTIC_STAGE || ''}>
                {props.analyticStages && props.analyticStages.map(row => {
                  return <option key={row.ANALYSIS_ID}
                    value={row.ANALYSIS_ID === null ? '' : row.ANALYSIS_ID}
                    label={row.ANALYSIS_STAGE} />
                })
                }
              </Field>
            </div>
            <div className="form-group col-md-4">
              ...
              <Field as="select" name="STATISTICIAN" placeholder="Statistician" className="form-control" id="statisticiansInput"
                     value={values.STATISTICIAN || ''}>
                {props.statisticians && props.statisticians.map(row => {
                  return <option key={row.STATISTICIAN_ID}
                    value={row.STATISTICIAN_ID === null ? '' : row.STATISTICIAN_ID}
                    label={row.STATISTICIAN_LNAME} />
                })
                }
              </Field>
            </div>
          ...
          <div className="form-row">
            <div className="form-group col-md-4">
              ...
              <Field as="select" name="PAPER_TYPE" placeholder="Paper Type" className="form-control" id="paperTypeInput"
                     value={values.PAPER_TYPE || ''}>
                {props.paperTypes && props.paperTypes.map(row => {
                  return <option key={row.PTYPE_ID}
                    value={row.PTYPE_ID === null ? '' : row.PTYPE_ID}
                    label={row.PAPER_TYPE} />
                })
                }
              </Field>
            </div>
          ...
          <div className="form-row">
            <div className="form-group col-md-4">
              <div className="form-check">
                <Field name="NDI_DATA_USED" className="form-control-sm form-check-input" id="ndiDataInput" type="checkbox" 
                       value={values.NDI_DATA_USED || ''}/>
                <label className="form-check-label" htmlFor="ndiDataInput">National Death Index<br></br> Data Used </label>
              </div>
            </div>
            <div className="form-group col-md-4">
              <div className="form-check">
                <Field name="SUPPLEMENTAL_FORMS_USED" className="form-control-sm form-check-input" id="supFormsInput" type="checkbox" 
                       value={values.SUPPLEMENTAL_FORMS_USED || ''}/>
                <label className="form-check-label" htmlFor="supFormsInput">Supplemental <br></br>Forms Used</label>
              </div>
            </div>
            <div className="form-group col-md-4">
              <div className="form-check">
                <Field name="DII_DATA_USED" className="form-control-sm form-check-input" id="diiDataInput" type="checkbox" 
                      value={values.DII_DATA_USED || ''}/>
                <label className="form-check-label" htmlFor="diiDataInput">Dietary Inflammatory Index<br></br> Data Used</label>
              </div>
            </div>
          </div>
...
{/*                <Select
                ...
                value={values.RELATED_PAPERS || ''}
                options={ props.options}
                className="basic-multi-select"
                classNamePrefix="select"
              />
 */}
                <Field as="select" name="RELATED_PAPERS" placeholder="Related Papers (Mark All)" className="form-control" id="relatedPapersInput" 
                     value={values.RELATED_PAPERS || ''}>
                {props.studies && props.studies.map(row => {
                  return <option key={row.STUDY_ID}
                    value={row.STUDY_ID === null ? '' : row.STUDY_ID}
                    label={row.STUDY_ABBR} />
                })
                }
              </Field>
            ...
            <div className="form-group col-md-3">
              ...
              <Field name="PP_PROPOSAL_APPROVAL" placeholder="PP Proposal Approval" className="form-control" id="proposalApprovalInput" 
                     value={values.PP_PROPOSAL_APPROVAL || ''}/>
            </div>
            <div className="form-group col-md-3">
              ...
              <Field name="PP_MANUSCRIPT_APPROVAL" placeholder="PP Paper Approval" className="form-control" id="manApprovalInput" 
                     value={values.PP_MANUSCRIPT_APPROVAL || ''}/>
            </div>
          </div>
          ...
          <div className="form-row">
            <div className="form-group col-md-4">
              ...
              <Field as="select" name="DATA_FOCUS" placeholder="Data Focus" className="form-control" id="dataFocusInput"
                     value={values.DATA_FOCUS || ''}>
                {props.dataFocus && props.dataFocus.map(row => {
                  return <option key={row.FOCUS_ID}
                    value={row.FOCUS_ID === null ? '' : row.FOCUS_ID}
                    label={row.FOCUS_NAME} />
                })
                }
              </Field>
            </div>
            <div className="form-group col-md-4">
              ...
              <Field as="select" name="PAPER_FOCUS" placeholder="Paper Focus" className="form-control" id="paperFocusInput"
                     value={values.PAPER_FOCUS || ''}>
                {props.paperFocus && props.paperFocus.map(row => {
                  return <option key={row.P_FOCUS_ID}
                    value={row.P_FOCUS_ID === null ? '' : row.P_FOCUS_ID}
                    label={row.P_FOCUS_NAME} />
                })
                }
              </Field>
            </div>
            <div className="form-group col-md-4">
              ...
              <Field as="select" name="SIGS" placeholder="SIGs" className="form-control" id="sigsInput"
                     value={values.SIGS || ''}>
                {props.sigs && props.sigs.map(row => {
                  return <option key={row.SIG_ID}
                    value={row.SIG_ID === null ? '' : row.SIG_ID}
                    label={row.SIG_NAME} />
                })
                }
              </Field>
          ...
          <div className="form-row">
            ...
            <div className="form-group col-md-2">
              ...
              <div className="form-check">
                ...
                <label className="form-check-label" htmlFor="newAuthorInput">Lead Author <br></br>New to WHI</label>
              </div>
            </div>
            <div className="form-group col-md-2">
              <br></br>
              <div className="form-check">
                ...
                <label className="form-check-label" htmlFor="earlyCareerInput">Early-career <br></br>investigator</label>
              </div>
            </div>
          </div>
          <br />
